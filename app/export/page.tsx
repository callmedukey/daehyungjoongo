...
const ExportPage = () => {
  return (
    <main className="px-4">
      <article className="relative min-h-[min(80dvh,600px)] mt-24 pt-12 sm:pt-16 xl:pt-24 text-center">
        <h1 className="text-primary thirty font-bold border-b-[3px] border-primaryFormText max-w-xl mx-auto">
          중고차 직수출의 신뢰와 전문성!
        </h1>
        <p className="about-max-width mt-12 text-left">
          중고차 수출은{" "}
          <span className="font-bold twenty">
            국내에서 판매하기 힘든 차량을
          </span>{" "}
          해외로 판매하는 효과적인 방법입니다. 해외 바이어와 직접 연결되어 중간
          마진을 제거하고, 차량 종류에 따라{" "}
          <span className="font-bold twenty">국내 시장보다 더 높은 가격에</span>{" "}
          판매할 수 있는 기회를 제공합니다. 더불어, 서류 절차가 간소화되어
          중고차 거래를 신속하고 편리하게 진행할 수 있습니다.
        </p>
        <div className="flex justify-center mt-[clamp(4rem,20vw,12rem)] items-center isolate lg:flex-row flex-col gap-4 sm:gap-8 lg:gap-0">
          <p className="shadow-xl border py-2 lg:p-4 bg-white z-10 max-w-[25rem] lg:min-w-[25rem] w-full   lg:translate-x-60 xl:translate-x-40 2xl:translate-x-24">
      ...
      <article className="shadow-lg mt-[clamp(4rem,20vw,10rem)] py-12 rounded-lg relative max-w-4xl mx-auto border border-b-0 isolate rounded-b-none px-4">
        <Image
          src={VectorShape}
          alt="벡터 모양"
      ...
      <div
        className="h-6 lg:h-12 w-full max-w-4xl mx-auto relative z-20 shadow-lg bg-white border border-t-0 rounded-lg rounded-t-none"
        aria-disabled={true}
      />
      <article className="mt-[clamp(4rem,20vw,10rem)]">
        ...
        <ul className="flex items-start justify-center gap-12 mt-12 max-w-7xl mx-auto flex-wrap">
          {secondList.map((item, index) => (
            ...
            >
              <p className="text-primaryFormText text-center thirty flex-1 basis-[5rem]">
                {item.title}
              </p>
              <p className="text-center flex-1">{item.text}</p>
              <Image
                src={item.src}
                alt={item.title}
                width={index === 0 ? 200 : 150}
                className={cn(
                  "flex-1 self-center xl:mt-16 lg:mt-12 mt-8",
      ...
      <article className="mt-[clamp(4rem,20vw,10rem)] relative rounded-lg max-w-7xl mx-auto overflow-clip pt-20 pb-20 md:pb-0 isolate md:px-24 px-4 flex justify-center items-center flex-col lg:flex-row">
        ...
        />
        <div className="bg-white max-w-[34rem] p-8 space-y-4 rounded-lg z-30 self-start xl:translate-x-12 md:-translate-y-4 mx-auto">
          <h3 className="thirty text-center font-bold text-primary">
            NO! 이제는 수출의 시대입니다!
          </h3>
        ...
        <Image
          ...
          alt="영업원"
          width={500}
          className="z-10 xl:-translate-x-12 self-end hidden md:block shrink-0"
          quality={100}
        />
      </article>
